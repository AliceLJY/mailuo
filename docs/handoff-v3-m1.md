...
**这份快照只有一个用途：给半 C 当对照样本。**
它不能用来做回归验证——见下面"关于回归怎么验"。

**已发现的质量事实（2026-08-29，首轮基线暴露的，记在这里备用）：**
`screenshot-1` 的「下周三下午3点」被解析成 `2026-08-31T15:00:00+08:00`。
基线时间是 2026-08-29（周六），下周三的正确答案是 **2026-09-02**，`2026-08-31` 是下周一，错两天。
而 `buildPerceptionSystemPrompt` 里明确塞了日历表并写了"从表里选日期，不要自己算星期几"。
这是 v2.0.0 生产代码就存在的缺陷，**本批不修**。
但文本路径跑通后，请用同一张图单独比一次同题：如果 DeepSeek 算对了，
把这个 case 写进 `docs/`——它是"识字与理解分离"这条架构的一条真收益证据
（视觉模型一边读图一边做日历推理，负担更重）。

**关于回归怎么验（2026-08-29 订正）：**
初版 Done when 第 4 条要求"改完 prompts.ts 后视觉路径抽取结果逐字节一致"，**这条不可能成立**：
`provider.ts` 的 `complete()` 已经把 `temperature` 默认设成 0，而 `qwen-vl-max` 是云端托管服务，
无 seed、无固定模型版本，同样的请求本来就会给出不同的响应。已实测两轮三张全部不一致。

也不要改成"验 HTTP 请求 fingerprint"——请求内容就是 prompt 加图片，源码没改它必然一致，验它捕获不到任何东西。

第 4 条真正要防的风险只有一个：**改 `prompts.ts` 新增文本版时，手滑把视觉版也改了。**
所以验的对象是 prompt 构建函数的返回字符串，不是模型响应。见 Done when 第 4 条。
...
4. **视觉提示词零改动回归**（纯函数快照，不调 API）：
   在动 `prompts.ts` 之前，把 `buildPerceptionSystemPrompt(now)` 与 `buildPerceptionUserPrompt()`
   的返回字符串落盘成快照，`now` 固定传 manifest 里那个 `2026-08-29T08:00:00+08:00`；
   加完文本版之后重跑，要求**逐字节一致**。
   这两个是纯函数，给定 `now` 输出确定，零随机、零 API 成本。有差异就是改错了地方。
   **不要用模型响应做这条回归**，理由见 Goal 第 4 节"关于回归怎么验"
