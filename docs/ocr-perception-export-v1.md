...
- `lines` 保留 ML Kit 每一行的原始 `text`、矩形 `x/y/w/h` 和 line confidence `conf`。坐标单位是原图像素，原点在左上角；confidence 不可用时写 `null`，不得编造或强制夹到 0–1。
- `side` 只能是 `me`、`them` 或 `null`。坐标同时命中左右对齐峰时使用局部像素采样；采样仍无法判断时保留 `null`。
- 每个无法完成气泡归属的采样问题都写入 `warnings`；零文本行补一条“未识别到文本行”。`degraded` 由 `warnings` 是否为空计算，不能由调用方单独指定。生产路径在降级时回退到 Qwen-VL；导出的仍是回退前原始 OCR 证据。
