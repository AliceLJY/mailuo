...
4. **LLM 层**（`server/src/llm/`）：`provider.ts` 抽象接口；`qwen.ts`（qwen-vl-max，走 dashscope 的 OpenAI 兼容端点，读 env `DASHSCOPE_API_KEY`；端点以官方文档为准，无法联网核对就用 `https://dashscope.aliyuncs.com/compatible-mode/v1`）；`deepseek.ts`（deepseek-chat，读 env `DEEPSEEK_API_KEY`）；`prompts.ts` 集中管理；所有 LLM 输出过 zod 校验，失败带错误信息重试一次，再失败抛明确错误不静默吞。**代码与文档绝不出现任何 key 真值**；提供 `server/.env.example`（只有键名）。
