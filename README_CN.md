...
- **分发形态路线图**（三级）：①自部署版（现状）——极客用户自建后端与 key；②**BYOK 单机版（v2.0 方向）**——agent 流水线与数据库整体移入 app 内，用户填自己的模型 API key 即可使用，无需任何服务器，档案数据只存在用户手机本地，隐私模型反而最优；③云服务版——多用户、账号体系与托管推理，属商业化范畴。过渡小步：应用内服务器地址设置页（一个通用 APK 服务所有自部署用户）。
