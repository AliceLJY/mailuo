...
**项目故事**：为什么做、怎么做的、踩了什么坑，见 [docs/story.md](docs/story.md)。
...
7. 有依据的洞察：生成关系解读、建议行动和话头，每条都必须引用 `based_on` observation 证据。洞察生成失败时，档案照常保存，可以在洞察页一键重新生成。
...
- 事项查重有意采用保守判据，可能漏掉相似项。
- 整张截图的重复上传：服务器模式已能识别文件完全相同的重复上传，不再调用模型、不再重复累计 observations，而是直接显示上次的结果。手机本地模式暂不支持，重复上传仍可能重复累计 observations。近似重复也还识别不了：同一段内容重新截一次图，或同一张截图经网页端与手机端各自压缩后文件不同，都会被当作新截图处理。
...
- 整张截图的重复检测与合并：手机本地模式的重复识别，以及重新截图、压缩方式不同等近似重复的识别与合并（服务器模式已能识别文件完全相同的重复上传）。
- 洞察定时主动推送。
- 系统日历集成。
- 消息应用直连。
- 群聊与多人参与场景优化。
