...
- **Fully-local route (server mode only)**: in server mode both providers speak the OpenAI-compatible API, so `DASHSCOPE_BASE_URL` / `DEEPSEEK_BASE_URL` can point at a local inference service (e.g. Ollama / vLLM). **The Android BYOK build currently connects to the official DashScope / DeepSeek endpoints only; custom base URLs are not yet supported there** (see `app/src/local/providers.ts`).
